...
完成一条任务（done / no-op / 自动合 main）后，**不要立即退出**，回到第三步再选一条。

### 链终止条件（任意一条命中即退出本 tick）
- 链长度达到 5 条
- 任意一条任务变成 spec_drafted（需要人审，再做也是写更多 spec）
- **连续 2 条任务 blocked**（说明大概率是基础设施级问题，如 pytest 全挂、主分支坏、依赖升级失败）
- **本 tick 累计 3 条任务 blocked**（即使不连续也异常）
- 第三步走到「都没有可做的」分支
- git pull --ff-only 失败（别的 cron 在抢，让位）

### 单条 blocked 不停链
- 第一次遇到 blocked 任务时，正常写 blocked_reason 推 dev（4c 失败分支的逻辑），然后**继续链**做下一条
- 内存里维护两个计数：本 tick total_blocked 和 consecutive_blocked
- 任务 done / no-op 时把 consecutive_blocked 重置为 0
- 任务 blocked 时 consecutive_blocked += 1，total_blocked += 1
